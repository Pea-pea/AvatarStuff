Shader "Hidden/Locked/.poiyomi/Poiyomi 8.1/Poiyomi Toon/f83263d17a4ff5745b46fea7c1e9855d"
{
	Properties
	{
 ...
 #define OPTIMIZER_ENABLED 
			...
			void ApplyAudioLinkDecal(in PoiMesh poiMesh, inout PoiFragData poiFragData, in PoiMods poiMods)
			{
				...
				float sn = sin(theta);
				uv = float2((uv.x - decalCenter.x) * cs - (uv.y - decalCenter.y) * sn + decalCenter.x, (uv.x - decalCenter.x) * sn + (uv.y - decalCenter.y) * cs + decalCenter.y);
				uv = remap(uv, float2(0, 0) - float4(1,1,1,-0.5).xz / 2 + float4(0.5,0.5,1,1), float4(1,1,1,-0.5).yw / 2 + float4(0.5,0.5,1,1), float2(0, 0), float2(1, 1));
				...
				if ((1.0 /*_ALDecalShapeClip*/))
				{
					...
					float bandwidth = (0.0 /*_ALDecalUVMode*/) == 1 ? (0.75 /*_ALDecalShapeClipBandWidth*/) / aluv.y : (0.75 /*_ALDecalShapeClipBandWidth*/);
			...
			float4 frag(VertexOut i, uint facing : SV_IsFrontFace) : SV_Target
			{
				...
				#endif
				poiFragData.baseColor = mainTexture.rgb * poiThemeColor(poiMods, float4(0.01771389,0.01728375,0.01728375,1).rgb, (0.0 /*_ColorThemeIndex*/));
				poiFragData.alpha = mainTexture.a * float4(0.01771389,0.01728375,0.01728375,1).a;
