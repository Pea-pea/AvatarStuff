Shader "Hidden/Locked/.poiyomi/Poiyomi 8.0/Poiyomi Toon/5851d5b9a02d903459d5873d0f5f1c94"
{
	Properties
	{
 ...
 #define PROP_CLIPPINGMASK 
			...
			{
				poiFragData.alpha = saturate(poiFragData.alpha + float(0.102));
			}
			...
			void calculateShading(inout PoiLight poiLight, inout PoiFragData poiFragData, in PoiMesh poiMesh, in PoiCam poiCam)
			{
				...
				if (float(1))
				{
					lns.x = poiEdgeNonLinearNoSaturate(poiLight.lightMap, float(0.333), float(1) * blurMap.r);
					lns.y = poiEdgeNonLinearNoSaturate(poiLight.lightMap, float(0.5), float(0.411) * blurMap.g);
					lns.z = poiEdgeNonLinearNoSaturate(poiLight.lightMap, float(0.25), float(0.1) * blurMap.b);
					lns.w = poiEdgeNonLinearNoSaturate(poiLight.lightMap, float(0.333), float(1) * blurMap.r, float(0));
				}
				else
				{
					lns.x = poiEdgeLinearNoSaturate(poiLight.lightMap, float(0.333), float(1) * blurMap.r);
					lns.y = poiEdgeLinearNoSaturate(poiLight.lightMap, float(0.5), float(0.411) * blurMap.g);
					lns.z = poiEdgeLinearNoSaturate(poiLight.lightMap, float(0.25), float(0.1) * blurMap.b);
					lns.w = poiEdgeLinearNoSaturate(poiLight.lightMap, float(0.333), float(1) * blurMap.r, float(0));
				}
				lns = saturate(lns);
				float3 indirectColor = 1;
				if (float4(0.5754442,0.5754442,0.5754442,1).a > 0)
				{
					#if defined(PROP_SHADOWCOLORTEX) || !defined(OPTIMIZER_ENABLED)
					...
					#endif
					indirectColor = lerp(float3(1, 1, 1), shadowColorTex.rgb, shadowColorTex.a) * float4(0.5754442,0.5754442,0.5754442,1).rgb;
				}
				...
				float3 mathRamp = lerp(float3(1, 1, 1), saturate(lerp((float4(1,0.8341731,0.7524076,1) * lerp(indirectColor, 1, float(0))), float3(1, 1, 1), saturate(poiLight.rampedLightMap))), float(1));
				...
				poiLight.rampedLightMap = poiLight.nDotLSaturated;
				float subsurface = 1;
				float skinScattering = saturate(subsurface * float(0) * 2);
				...
				poiLight.finalLighting = min(lerp(poiLight.indirectColor * float4(1,0.8341731,0.7524076,1), float4(1,0.8341731,0.7524076,1), float(0)) + (sss * poiLight.directColor), poiLight.directColor);
				#endif
				#ifdef _LIGHTINGMODE_SDF
				...
				float3 indirectColor = float4(1,0.8341731,0.7524076,1).rgb;
				indirectColor = indirectColor * lerp(poiLight.indirectColor, poiLight.directColor, float(0));
				...
				if (float(0) == 1)
				{
					...
					float passthrough = 0;
					#else
					float passthrough = float(0.469);
					#endif
					if (float(0.239) == float(0)) float(0.239) += 0.001;
					poiLight.rampedLightMap = smoothstep(float(0.239), float(0), 1 - (.5 * poiLight.nDotL + .5));
					#if defined(POINT) || defined(SPOT)
					poiLight.finalLighting = lerp(poiLight.directColor * max(min(poiLight.additiveShadow, poiLight.detailShadow), passthrough), poiLight.indirectColor, smoothstep(float(0), float(0.239), 1 - (.5 * poiLight.nDotL + .5))) * poiLight.attenuation;
					#else
					poiLight.finalLighting = lerp(poiLight.directColor * max(min(poiLight.attenuation, poiLight.detailShadow), passthrough), poiLight.indirectColor, smoothstep(float(0), float(0.239), 1 - (.5 * poiLight.nDotL + .5)));
					#endif
				}
				if (float(0) == 2)
				{
				...
				for (int index = 0; index < 4; index++)
				{
					...
					if (float(0) == 1) // Toon
					{
						vertexLighting += lerp(poiLight.vColor[index] * poiLight.vAttenuation[index], poiLight.vColor[index] * float(0.469) * poiLight.vAttenuation[index], smoothstep(float(0), float(0.239), .5 * poiLight.vDotNL[index] + .5)) * poiLight.detailShadow;
					}
				}
			...
			{
				float enviroRimAlpha = saturate(1 - smoothstep(min(float(0), float(0.715)), float(0.715), poiCam.vDotN));
			...
			float4 frag(v2f i, uint facing : SV_IsFrontFace) : SV_Target
			{
				...
				#else
				poiLight.indirectColor = lerp(0, poiLight.directColor, float(0.469));
				#endif
