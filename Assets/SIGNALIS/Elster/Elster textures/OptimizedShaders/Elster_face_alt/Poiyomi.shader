Shader "Hidden/Locked/.poiyomi/Poiyomi 8.0/Poiyomi Toon/74ef84c683b98d64db49028743ed5e68"
{
	Properties
	{
 ...
 #define PROP_EMISSIONMAP 
			...
			void applyShadeMapping(inout PoiFragData poiFragData, PoiMesh poiMesh, inout PoiLight poiLight)
			{
				...
				mainShadowMask *= poiLight.shadowMask * float(0.981);
				firstSecondShadowMask *= poiLight.shadowMask * float(0.981);
				if (float(0) == 0)
				{
			...
			void calculateShading(inout PoiLight poiLight, inout PoiFragData poiFragData, in PoiMesh poiMesh, in PoiCam poiCam)
			{
				...
				float shadowStrength = float(0.981) * poiLight.shadowMask;
				#ifdef POI_PASS_OUTLINE
				...
				if (float(0))
				{
					lns.x = poiEdgeNonLinearNoSaturate(poiLight.lightMap, float(0.192), float(1) * blurMap.r);
					...
					lns.w = poiEdgeNonLinearNoSaturate(poiLight.lightMap, float(0.192), float(1) * blurMap.r, float(0));
				}
				else
				{
					lns.x = poiEdgeLinearNoSaturate(poiLight.lightMap, float(0.192), float(1) * blurMap.r);
					...
					lns.w = poiEdgeLinearNoSaturate(poiLight.lightMap, float(0.192), float(1) * blurMap.r, float(0));
				}
				lns = saturate(lns);
				float3 indirectColor = 1;
				if (float4(0.7986599,0.5707019,0.5056303,1).a > 0)
				{
					#if defined(PROP_SHADOWCOLORTEX) || !defined(OPTIMIZER_ENABLED)
					...
					#endif
					indirectColor = lerp(float3(1, 1, 1), shadowColorTex.rgb, shadowColorTex.a) * float4(0.7986599,0.5707019,0.5056303,1).rgb;
				}
				...
				indirectColor = indirectColor * lerp(poiLight.indirectColor, poiLight.directColor, float(0.196));
				...
				float3 mathRamp = lerp(float3(1, 1, 1), saturate(lerp((float4(1,0.8341731,0.7524076,1) * lerp(indirectColor, 1, float(0.196))), float3(1, 1, 1), saturate(poiLight.rampedLightMap))), float(0.981));
				...
				poiLight.finalLighting = finalWrap * saturate(mathRamp + 1 - float(0.981));
				#endif
				...
				poiLight.finalLighting = min(lerp(poiLight.indirectColor * float4(1,0.8341731,0.7524076,1), float4(1,0.8341731,0.7524076,1), float(0.196)) + (sss * poiLight.directColor), poiLight.directColor);
				#endif
				#ifdef _LIGHTINGMODE_SDF
				...
				indirectColor = indirectColor * lerp(poiLight.indirectColor, poiLight.directColor, float(0.196));
				indirectColor = lerp(poiLight.directColor, indirectColor, float(0.981) * poiLight.shadowMask);
				...
				for (int index = 0; index < 4; index++)
				{
					...
					if (float(0) == 1) // Toon
					{
						vertexLighting += lerp(poiLight.vColor[index] * poiLight.vAttenuation[index], poiLight.vColor[index] * float(0.469) * poiLight.vAttenuation[index], smoothstep(float(0), float(0.5), .5 * poiLight.vDotNL[index] + .5)) * poiLight.detailShadow;
					}
				}
			...
			{
				float3 emission0 = 0;
				float emissionStrength0 = float(10.44);
